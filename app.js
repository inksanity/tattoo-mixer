...
function refineWeightsForPigments(weights, subset, targetHex){
  try{
    if(typeof TLColor==='undefined' || !TLColor.refineWeightsSubtractive) return weights;
    const inksHex = subset.map(ink=>ink.hex);
    return TLColor.refineWeightsSubtractive(inksHex, weights, targetHex, 80, 0.2);
  }catch(e){ console.warn('Refine failed', e); return weights; }
}
function computeAllRecipes(){ const inks=BRANDS[state.brand]; const capDrops={S:20,M:40,L:80}[state.cap]; const results=state.swatches.map(hx=>{ const subset=pickSubset(hx,inks); let w=solveWeights(hexToRgb(hx),subset);
    w = refineWeightsForPigments(w, subset, hx); const comp=subset.map((ink,i)=>({ink,w:w[i]})).filter(x=>x.w>0.02).sort((a,b)=>b.w-a.w).slice(0,4); const sum=comp.reduce((s,c)=>s+c.w,0)||1; comp.forEach(c=>c.w/=sum); const raw=comp.map(c=>c.w*capDrops); const drops=raw.map(x=>Math.max(0,Math.round(x))); let diff=capDrops - drops.reduce((s,x)=>s+x,0); while(diff!==0){ const idx=drops.indexOf(Math.max(...drops)); drops[idx]+= (diff>0?1:-1); diff=capDrops - drops.reduce((s,x)=>s+x,0); } comp.forEach((c,i)=> c.drops=drops[i]); return {hex:hx,comp,total:capDrops}; }); state._results=results; renderRecipes(); }
