...
const historyStack=[]; function pushHistory(){ historyStack.push([...state.swatches]); if(historyStack.length>50) historyStack.shift(); } $('btnUndo').onclick=()=>{ if(!historyStack.length) return; state.swatches=historyStack.pop(); renderPalette(); computeAllRecipes(); drawMarkers(); };

Object.keys(BRANDS).forEach(b=>{ const o=document.createElement('option'); o.value=b; o.textContent=b; brandSel.appendChild(o); }); brandSel.value=state.brand;

// Tier
segBasic.onclick=()=>setTier('basic'); segPro.onclick=()=>setTier('pro');
function setTier(t){ state.tier=t;
...
btnUpgrade.onclick=()=>{ modal.style.display='flex'; }; closeModal.onclick=()=>{ modal.style.display='none'; };
...
// Auto-extract
btnExtract.onclick=()=>{ if(state.tier==='basic'){ flash('Pro only'); return; } if(canvas.width===0){ flash('Load an image first'); return; } const k=parseInt(kSel.value,10); const {colors,coords}=extractPaletteWithCoords(canvas,k,biasPure.checked); state.swatches=colors.map(rgbToHex); state.markers=coords.map(([x,y])=>({x,y})); renderPalette(); computeAllRecipes(); drawMarkers(); };
...
canvas.addEventListener('mousemove', e=>{ if(state.eyedropper) updateLoupe(e.clientX,e.clientY); }); canvas.addEventListener('mouseleave',()=>{ loupe.style.display='none'; }); canvas.addEventListener('click', e=>{ if(state.eyedropper) pickAt(e.clientX,e.clientY); });
...
function renderPalette(){ paletteStrip.innerHTML=''; const style = (state.tier==='basic') ? 'pngSquare' : state.style; state.swatches.forEach((hx,idx)=>{ let el; if(style==='pngSquare'){ el=document.createElement('div'); el.className='square'; el.style.setProperty('--chipColor',hx);} else if(style==='pngSplat'){ el=document.createElement('div'); el.className='splat'; el.style.setProperty('--chipColor',hx);} else { el=document.createElement('div'); el.className='svgSplat'; const svg=document.createElementNS('http://www.w3.org/2000/svg','svg'); svg.setAttribute('viewBox','0 0 100 100'); svg.style.width='100%'; svg.style.height='100%'; const path=document.createElementNS('http://www.w3.org/2000/svg','path'); path.setAttribute('fill',hx); path.setAttribute('d',CUSTOM_SPLAT_PATH); svg.appendChild(path); el.appendChild(svg); } const label=document.createElement('div'); label.className='slabel'; label.textContent=hx; el.appendChild(label); el.onclick=()=>{ state.selectedSwatch=idx; drawMarkers(); }; // long-press & context delete
  let timer=null; el.addEventListener('touchstart',()=>{ timer=setTimeout(()=>{ pushHistory(); state.swatches.splice(idx,1); state.selectedSwatch=-1; renderPalette(); computeAllRecipes(); drawMarkers(); },550);},{passive:true}); el.addEventListener('touchend',()=>clearTimeout(timer),{passive:true}); el.addEventListener('touchmove',()=>clearTimeout(timer),{passive:true}); el.addEventListener('contextmenu',(e)=>{ e.preventDefault(); pushHistory(); state.swatches.splice(idx,1); state.selectedSwatch=-1; renderPalette(); computeAllRecipes(); drawMarkers(); }); paletteStrip.appendChild(el); }); }
...
function pickSubset(targetHex,brandInks){ const targetLab=rgbToLab(hexToRgb(targetHex)); const arr=brandInks.map(ink=>({ink,d:distLab(targetLab,rgbToLab(hexToRgb(ink.hex)))})); arr.sort((a,b)=>a.d-b.d); const chosen=arr.slice(0,3).map(x=>x.ink); const L=targetLab[0]; const hasWhite=brandInks.find(i=>/white/i.test(i.name)), hasBlack=brandInks.find(i=>/black/i.test(i.name)); if(L>70 && hasWhite && !chosen.includes(hasWhite)) chosen.push(hasWhite); if(L<30 && hasBlack && !chosen.includes(hasBlack)) chosen.push(hasBlack); return chosen.slice(0,4); }
function computeAllRecipes(){ const inks=BRANDS[state.brand]; const capDrops={S:20,M:40,L:80}[state.cap]; const results=state.swatches.map(hx=>{ const subset=pickSubset(hx,inks); const w=solveWeights(hexToRgb(hx),subset); const comp=subset.map((ink,i)=>({ink,w:w[i]})).filter(x=>x.w>0.02).sort((a,b)=>b.w-a.w).slice(0,4); const sum=comp.reduce((s,c)=>s+c.w,0)||1; comp.forEach(c=>c.w/=sum); const raw=comp.map(c=>c.w*capDrops); const drops=raw.map(x=>Math.max(0,Math.round(x))); let diff=capDrops - drops.reduce((s,x)=>s+x,0); while(diff!==0){ const idx=drops.indexOf(Math.max(...drops)); drops[idx]+= (diff>0?1:-1); diff=capDrops - drops.reduce((s,x)=>s+x,0); } comp.forEach((c,i)=> c.drops=drops[i]); return {hex:hx,comp,total:capDrops}; }); state._results=results; renderRecipes(); }
function renderRecipes(){ recipes.innerHTML=''; const unit= state.tier==='basic' ? 'drops' : unitSel.value; state._results.forEach(res=>{ const card=document.createElement('div'); card.className='recipe'; const sw=document.createElement('div'); sw.className='sw'; sw.style.background=res.hex; const body=document.createElement('div'); const hdr=document.createElement('div'); hdr.style.fontWeight='700'; hdr.textContent=res.hex; const meta=document.createElement('div'); meta.className='hint'; meta.textContent=`${state.brand} • ${state.cap} • total ${res.total} drops`; const list=document.createElement('div'); list.className='inkrow'; res.comp.forEach(c=>{ let value=''; if(unit==='drops') value=`${c.drops} drops`; else if(unit==='percent') value=`${Math.round(c.w*100)}%`; else if(unit==='ml') value=`${(c.drops*0.05).toFixed(2)} ml`; else if(unit==='oz') value=`${(c.drops*0.05/29.5735).toFixed(3)} oz`; const chip=document.createElement('div'); chip.className='chip'; chip.textContent=`${c.ink.name}: ${value}`; list.appendChild(chip); }); body.appendChild(hdr); body.appendChild(meta); body.appendChild(list); card.appendChild(sw); card.appendChild(body); recipes.appendChild(card); }); }
function flash(m){ statusChip.textContent=m; setTimeout(()=> statusChip.textContent='Ready',1100); }

// Save/Load & Export
btnSave.onclick=()=>{ if(state.tier==='basic'){ flash('Pro only'); return; } const payload={brand:state.brand,cap:state.cap,unit:unitSel.value,swatches:state.swatches,markers:state.markers,studio:studioInput.value,client:clientInput.value,date:dateInput.value,placement:placementInput.value}; const blob=new Blob([JSON.stringify(payload)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='tattoo_lab_session.tatmix'; a.click(); };
...
function buildPages(includeMarkers){ const A4W=1654,A4H=2339; const pages=[]; let idx=0; const perPageHeader=720,itemH=96; const maxPerPage=Math.max(1,Math.floor((A4H-perPageHeader)/itemH)); while(idx<state._results.length || (idx===0 && state._results.length===0)){ const c=document.createElement('canvas'); c.width=A4W; c.height=A4H; const cx=c.getContext('2d'); cx.fillStyle=root.classList.contains('light')?'#ffffff':'#0b0e13'; cx.fillRect(0,0,A4W,A4H); cx.fillStyle=root.classList.contains('light')?'#0b1220':'#eef2f8'; cx.font='bold 42px Inter, Arial'; cx.fillText('Tattoo Lab — Mix Sheet',64,86); cx.font='16px Inter, Arial'; cx.fillText(`Brand: ${state.brand}   Cap: ${state.cap}   Units: ${state.tier==='basic'?'drops':unitSel.value}`,64,126); const metaY=164; cx.font='14px Inter, Arial'; cx.fillText(`Studio/Artist: ${studioInput.value||'-'}`,64,metaY); cx.fillText(`Client: ${clientInput.value||'-'}`,64,metaY+22); cx.fillText(`Date: ${dateInput.value||'-'}`,64,metaY+44); cx.fillText(`Placement: ${placementInput.value||'-'}`,64,metaY+66); const refX=64,refY=metaY+96,refW=A4W-128,refH=420; cx.strokeStyle=root.classList.contains('light')?'#dbe1ec':'#1f2736'; cx.lineWidth=2; cx.strokeRect(refX,refY,refW,refH); if(canvas.width>0){ const scale=Math.min(refW/canvas.width,refH/canvas.height); const w=Math.floor(canvas.width*scale), h=Math.floor(canvas.height*scale); const ox=refX+Math.floor((refW-w)/2), oy=refY+Math.floor((refH-h)/2); cx.drawImage(canvas,0,0,canvas.width,canvas.height,ox,oy,w,h); if(includeMarkers){ for(let i=0;i<state.markers.length;i++){ const m=state.markers[i]; const mx=ox+m.x*w, my=oy+m.y*h; cx.fillStyle='#ffffff'; cx.beginPath(); cx.arc(mx,my,14,0,Math.PI*2); cx.fill(); cx.lineWidth=3; cx.strokeStyle='#ff7a1a'; cx.stroke(); cx.fillStyle='#0b1220'; cx.font='bold 16px Inter, Arial'; cx.textAlign='center'; cx.textBaseline='middle'; cx.fillText(String(i+1),mx,my+1); cx.textAlign='start'; cx.textBaseline='alphabetic'; } } } let px=64,py=refY+refH+24; const step=140,size=64; const maxShow=Math.min(10,state.swatches.length); for(let i=0;i<maxShow;i++){ const hx=state.swatches[i]; cx.fillStyle=hx; cx.strokeStyle=root.classList.contains('light')?'#dbe1ec':'#1f2736'; cx.lineWidth=2; cx.beginPath(); cx.roundRect(px,py,size,size,14); cx.fill(); cx.stroke(); cx.fillStyle=root.classList.contains('light')?'#0b1220':'#eef2f8'; cx.font='12px Inter, Arial'; cx.fillText(hx,px,py+size+18); px+=step; if(px > A4W-64-step){ px=64; py += size+36; } } let ry=perPageHeader; const start=idx; const end=Math.min(state._results.length,idx+maxPerPage); for(let j=start;j<end;j++){ const res=state._results[j]; cx.fillStyle=res.hex; cx.strokeStyle=root.classList.contains('light')?'#dbe1ec':'#1f2736'; cx.lineWidth=2; cx.beginPath(); cx.roundRect(64,ry+10,48,48,10); cx.fill(); cx.stroke(); cx.fillStyle=root.classList.contains('light')?'#0b1220':'#eef2f8'; cx.font='bold 18px Inter, Arial'; cx.fillText(res.hex,64+60,ry+32); cx.font='12px Inter, Arial'; cx.fillText(`${state.brand} • ${state.cap} • total ${res.total} drops`,64+60,ry+52); let cx0=64+60, cy0=ry+72, chipH=22; res.comp.forEach(c=>{ const label=`${c.ink.name}: ${c.drops} drops`; const mw=Math.max(110,Math.ceil(cx.measureText(label).width)+20); cx.fillStyle=root.classList.contains('light')?'#f3f5f9':'#0c0f15'; cx.strokeStyle=root.classList.contains('light')?'#c8d2e6':'#2a354b'; cx.lineWidth=1.5; cx.beginPath(); cx.roundRect(cx0,cy0,mw,chipH,11); cx.fill(); cx.stroke(); cx.fillStyle=root.classList.contains('light')?'#0b1220':'#eef2f8'; cx.font='12px Inter, Arial'; cx.fillText(label,cx0+10,cy0+15); cx0 += mw + 10; if(cx0 > (A4W-64-110)){ cx0 = 64+60; cy0 += chipH + 8; } }); ry += 96; } pages.push(c); idx=end; if(idx>=state._results.length) break; } return pages; }
...
btnSplatConfig.onclick=()=>{ if(state.tier==='basic'){ flash('Pro only'); return; } $('splatInput').value=CUSTOM_SPLAT_PATH||''; $('splatModal').style.display='flex'; };
$('splatSave').onclick=()=>{ CUSTOM_SPLAT_PATH=$('splatInput').value.trim()||null; if(CUSTOM_SPLAT_PATH){ localStorage.setItem('tl-splat',CUSTOM_SPLAT_PATH);} renderPalette(); flash('Splat path saved'); };
$('splatClear').onclick=()=>{ localStorage.removeItem('tl-splat'); CUSTOM_SPLAT_PATH=null; renderPalette(); flash('Splat path cleared'); };
