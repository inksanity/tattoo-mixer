/* Tattoo Lab - colorEngine.js (stable) */
(function(global){
  ...
  function refineWeightsSubtractive(inksHex, initW, targetHex, iters=60, step=0.18){
    ...
    const inks01 = inksHex.map(h=>to01(hx(h)));
    const tgt01 = to01(hx(targetHex)); const tgtLab = srgb01ToOkLab(tgt01);
    for(let it=0; it<iters; it++){
      const mix01 = mixSubtractiveCMY(inks01, w);
      const base = okLabDeltaE(srgb01ToOkLab(mix01), tgtLab);
      const g = new Array(w.length).fill(0), eps=1e-2;
